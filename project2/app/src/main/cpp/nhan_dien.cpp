...
std::vector<matrix<float, 0, 1>> detect(
        dlib::array2d<unsigned char>& images
)
{
    ...
    for (auto face : detector(images))
    {
        auto shape = sp(images, face);
        matrix<rgb_pixel> face_chip;
        extract_image_chip(images, get_face_chip_details(shape, 150, 0.25), face_chip);
